...
public:
	...
	void decode_seek(double p_seconds,abort_callback & p_abort) {
		...
		// get_size_ex fails if size is not known (where get_size would return filesize_invalid). Should never fail on seekable streams (if it does it's not our problem anymore).
		t_filesize max = m_file->get_size_ex(p_abort);
		if (target > max) target = max;//clip seek-past-eof attempts do legal range (next decode_run() call will just signal EOF).
